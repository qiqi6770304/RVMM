function PopNew = KrigingSelect(PopDec,PopObj,MSE,V,V0,NumV1,delta,mu_,theta,per,PopDec1,PopObj1,MSE1,V1,A2,A2Obj)
...
wholeobj =wholeobj(NDSort(wholeobj,1)==1,:);
...
if ~isempty(PopObj)
    ...
    if  1==1 && IGD((max(allR-zmin,0))./scale,max( A2Obj- zmin,0)./scale) < IGD((max(allR-zmin,0))./scale,max(A2Obj - Zmin,0)./scale)
...
if 1==1
    ...
    if ~isempty(find(frontNo(2:end)==2))&&frontNo(1)==1
